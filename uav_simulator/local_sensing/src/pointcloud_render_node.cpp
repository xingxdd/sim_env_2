...
ros::Publisher pub_global_cloud, pub_local_cloud;
...
pcl::PointCloud<pcl::PointXYZ> _cloud_all_map, _local_map_in_global, _local_map_in_local;
pcl::VoxelGrid<pcl::PointXYZ> _voxel_sampler;
sensor_msgs::PointCloud2 _local_map_global_pcd, _local_map_local_pcd;
...
void renderSensedPoints(const ros::TimerEvent& event) {
  if(!has_odom){
    ROS_ERROR("no odom, please check!");
    return;
  }
  if(!has_global_map){
    ROS_ERROR("no global map, please check!");
    return;
  }
  ...
  _local_map_in_global.points.clear();
  _local_map_in_local.points.clear();
  ...
  if (_kdtreeLocalMap.radiusSearch(searchPoint, sensing_horizon,
    ...
    for (size_t i = 0; i < _pointIdxRadiusSearch.size(); ++i) {
      ...
      _local_map_in_global.points.push_back(pt);
      pt.x -= _odom.pose.pose.position.x;
      pt.y -= _odom.pose.pose.position.y;
      pt.z -= _odom.pose.pose.position.z;
      _local_map_in_local.push_back(pt);
    }
  } else {
    return;
  }

  _local_map_in_global.width = _local_map_in_global.points.size();
  _local_map_in_global.height = 1;
  _local_map_in_global.is_dense = true;

  _local_map_in_local.width = _local_map_in_local.points.size();
  _local_map_in_local.height = 1;
  _local_map_in_local.is_dense = true;

  pcl::toROSMsg(_local_map_in_global, _local_map_global_pcd);
  _local_map_global_pcd.header.frame_id = "world";

  pcl::toROSMsg(_local_map_in_local, _local_map_local_pcd);
  _local_map_local_pcd.header.frame_id = "map";

  pub_local_cloud.publish(_local_map_local_pcd);

  pub_global_cloud.publish(_local_map_global_pcd);
}
...
int main(int argc, char** argv) {
  ...
  pub_global_cloud =
      nh.advertise<sensor_msgs::PointCloud2>("/pcl_render_node/cloud_in_global", 10);
  pub_local_cloud =
      nh.advertise<sensor_msgs::PointCloud2>("/pcl_render_node/cloud_in_local", 10);
